...
function fallbackResponse(prompt: string): string {
  const shortPrompt = extractTextFromPrompt(prompt);
  if (/question/i.test(prompt)) {
    return `What is the key idea you would use to explain ${shortPrompt}?`;
  }
  ...
  return `Here is a clear explanation of ${shortPrompt}. Focus on the definition, the underlying pattern, and one concrete example.`;
}
